...
>
> 当前基线：2026-08-12，当前 HEAD + working tree。
>
...
图片：`src` 使用 Unreal 软对象路径，例如 `/Game/UI/T_Logo.T_Logo`；不支持磁盘图片和 HTTP 下载。仅影响 Paint 的 Pseudo State 变化会保留已加载图片资源和对应 Brush，不重新同步加载或重建无关图片；Resource 影响、绑定或完整文档刷新仍可能同步加载，编译依赖与完整 Resource Cache 属于 M2.4。
