...
>
> 当前里程碑：M4——UE 原生表现与合成 6 / 9；M4.6 Animation Kernel 已完成，本轮停止
>
> 当前交付 Profile：PersonalGame-ready 0.5——Win64 项目内生产使用；通用商业 1.0 延后
>
> 最近核验：2026-08-22 `2d2be76` 完成版本化 Animation IR 1.0、Session-owned active-only Track、精确注入 Clock、单地址 Retarget/Replace/Cancel、Generation/Session 清理与独立 Packaged smoke；117 / 117 Automation、UE 5.8 Win64 Editor Development 5 / 5、Development/Shipping BuildCookRun Operations `9ad907d4395f4411828cb837a946810b` / `f21a6db2321d4f4dbbfcbbfc605599de` 与双配置独立 Animation smoke 全部通过
>
...
| 当前最大风险 | M2 风险已降至当前 Win64 项目内 0.5 可接受等级；M3 基础合同、M4 Resource/Material/Feedback、M4.5 Transform/Clip 与 M4.6 原生 Animation Kernel 已收口。M4 剩余最大架构风险转为 Transition 作者 lowering/View 接入与真实 Portal/离屏 Compositing；Behavior/FieldNotify/Command Adapter、Stable Identity 接入和真实双 LocalPlayer/CommonUI Modal 仍开放 | 🟡 |
| 当前策略 | 保持 M2 出口门、M3 合同边界和已完成的 M4.1～M4.6 纵向闭环；本轮在 M4.6 停止，下一建议仅为 M4.7 Transition Lowering，未激活 Compositing 或相邻能力 | ✅ |

### 2.2 验证快照

| 项目 | 当前值 |
| --- | --- |
| 自动化测试 | 117 / 117 通过、0 failed、0 skipped、0 warnings（12.369 秒；2026-08-22，M4.6；`StartsWith:WebToUE`）；最终 Animation/CompiledDocument 聚焦 5 / 5（0.044 秒） |
| 当前编译 | M4.6 最终 UE 5.8 Win64 Editor Development Operation `d7908002f48a4538817f44eb7fb6e66a` 完成 5 / 5 actions、warnings-as-errors并启动 PID `7740`；最终 Shipping 发布门恢复 Editor PID `8792`，VibeUE 5.0 readiness/MCP HTTP 200 健康 |
| 当前发布 | tracked Win64 Development/Shipping BuildCookRun Operations `9ad907d4395f4411828cb837a946810b` / `f21a6db2321d4f4dbbfcbbfc605599de` 均通过，各 598 packages、2,267 WebToUE content chunks（含 global 2,268）、251.98 MiB、AutomationTool 最终 ExitCode 0；双配置 Animation smoke 均 `success=true`，记录 IR 1.0、Test Clock、idle ticker 0、quarter sample 2.5、retarget midpoint 11.25、cancel underlying 77、9 次事务、10 条有界 trace 与 3 次 lease release；它们不证明视觉输出、Renderer/GPU 或 input-to-pixel 性能 |
| 历史发布 | Win64 Game Development/Shipping、BuildCookRun、BuildPlugin 曾通过；发布前须在当前提交重跑 |
| Git 基线 | M2.9 检查点为 `8d26643`、`36fc522`、`e8ac686`、`e524997`、`64c2f7e`、`7df6832`；M3.0 为 `66ada22`、`ecb094c`；M3.1 为 `1fc2a20`、`5342bf6`、`3f1bf20`；M3.2 为 `7be0f8e`；M3.3 为 `692ffeb`、`048a676`、`a7c1db4`；M3.4 为 `9c33768`、`d3e1b80`、`2e0f1c3`；M3.5 为 `4001b08`、`4bdf63c`；M3.6 为 `f05ce52`、`0ae3ac4`；M3.7 为 `21f2933`、`f1c2e01`；M3.8 为 `c3383c6`、`9b666b3`；M3.9 为 `f60b94d`；M4.1 为 `6903e34`、`ccddf69`、`99ad868`、`507c616`、`394cb94`；M4.2 为 `3cce712`、`9449bee`、`be4e728`、`2184640`、`4eacabc`；M4.3a 为 `a046603`、`1df1704`；M4.3b 为 `e4d4ba4`；M4.4 为 `b1a95df`、`ac6314b`、`5730cd3`、`05359ee`；M4.5 实现与门修复为 `f55d4ec`、`b0ad573`、`f6a825c`；M4.6 实现为 `2d2be76` |
...
| M4 UE 原生表现与合成 | 🚧 | 6 / 9 | Texture、静态 Material Brush、动态 MID 参数、Feedback Profile/Router、Visual Transform/Clip 与 Animation Kernel 已完成；之后为 Transition Lowering、Compositing 与性能出口门 |
...
| 层 | 当前实现 | 已接受的后续边界 |
| --- | --- | --- |
| Compiler | Parser/CSS 前端、RichText lowering、Property ID/Typed Value、Property Metadata、固定槽位 Typed Cascade、Property 应用、Yoga Adapter、C++ Interop Schema snapshot/`.d.ts` 投影；Texture importer 支持 Unreal、相对 Source 与 `generated:`，静态 Material importer 支持 `/Game`/`/Engine` `UMaterialInterface`，并生成类型化 ResourceId/provenance/Brush metadata、BLAKE3-256 Source/saved-package closure、freshness stamp 与 Animation IR 1.0 seal | 受限 Behavior/TSX 静态编译消费 Schema；Material 参数/Transition 作者 lowering、Behavior IR、Source Map 与完整 Incremental/DDC/Lockfile |
| Asset/IR | 私有扁平 CompiledNodes/Rules、类型化样式声明与 Texture/Material/Font/String Table Resource Manifest、自定义版本 13、原子提交；Texture 与静态 Material ResourceId/provenance/residency/必要 Brush metadata、Resource IR 1.2/freshness stamp、类型化 Visual Transform/Origin，以及版本化 Animation IR 1.0 已序列化 | Behavior/Capability IR、Transition/作者 lowering、Material 参数作者声明与完整多层迁移 |
| Session/Runtime | Screen UI Session 绑定 LocalPlayer、World、Surface、Data/Command、Environment、显式 Game/Unscaled/Real/Test Clock 与 Session Generation；Session-owned 更新协调器在 Game Thread 原子提交，异步协调器提供无默认 Tick 的 Timer/Command Result，Animation 协调器提供 active-only Track、单地址 lease、Retarget/Replace/Cancel、精确 Clock sample、Generation/Session cleanup 与有界 Trace；Property Ownership Policy 区分 CSS baseline、唯一 Binding/Behavior durable owner、Animation overlay 和 restrictive gate | 把 FieldNotify/类型化 Command/Behavior 接入事务与异步结果；让 M4.7 作者/Compiler/View 消费 Animation Kernel，并让 M5/M6 消费 Stable Identity 与多树合同 |
...
| Presentation | Handle-keyed Layout/Text/Paint/Brush、ResourceId-indexed View-owned 状态/强对象/异步句柄与可 patch Display List；Critical/Visible/Lazy Resource 边界保持不变；Texture、静态 Material 和 View/Node-owned MID 已接入。Translate/Scale/Rotate/Origin 解析为 affine LocalToView，祖先变换和滚动形成 Clip Chain；Paint 使用 Slate render transform/任意 quad clip，Semantic 使用完整变换后 border-box AABB，空间索引使用变换且裁剪后的可见 AABB，精确命中先检查 clip quads 再逆变换到本地 border box，K=1 hover 只 patch 受影响子树 | Animation target adapter 尚未接入现有 View/Paint，Material 参数作者语法、Transition/Time、Route、真实 Portal/Overlay、分级 Compositing 与更大资源 Corpus 的真实释放/内存门 |
...
Cooked 游戏保留 Compiled Nodes/Rules、Root、Animation IR 1.0、Texture/Font/String Table Resource Manifest、诊断和 Runtime 模块；HTML/CSS 原文、源路径和依赖文件属于 Editor-only 数据。
...
| R-05 | Compiler/View 职责集中 | Low | Core 服务、Runtime Presentation、Semantic/Focus/Schema/Animation 接口、Editor-only `.d.ts` emitter、Game-owned Packaged Runner 与独立出口门脚本已拆分；当前精确发现 117 项 Automation | 后续能力进入对应服务 | ✅ Mitigated |
...
| R-07 | Map 声明丢失重复属性顺序 | Medium | Core、Compiled IR 与 Hydration 已使用有序声明；当前 117/117 Automation 与 M4.6 Win64 Development/Shipping BuildCookRun 通过 | 保持 Ordered Declaration 专项与资产版本门 | ✅ Mitigated |
...
| R-16 | Transform、Opacity、Material、Clip 与 Filter 需要 Stacking/Compositing，而扁平 Display List 语义不足 | High | `f05ce52` + ADR-0007 冻结投影；M4.3a/M4.3b 已证明 Material/MID，M4.5 已证明 Transform/Clip，M4.6 又以双配置 Packaged kernel gate 证明单一 Animation IR/Track、Clock、lease、Retarget/Replace/Cancel 与生命周期清理；作者 Transition/View adapter、Portal 合成和离屏层仍无证据 | M4.7～M4.8 只在 M4.6 Kernel 上收口 Transition 与分级 Compositing；禁止把受控 kernel 工作量外推为视觉、Portal、离屏 Filter、GPU 成本或产品性能等价 | 🚧 M4 |
...
- [ ] `P0.5` 将 Opacity/Color/Transform Typed Transition 降低到同一 Animation IR/Track，提供受控 easing、Retarget/Reverse/Cancel/Fill 语义；不得先建立独立临时执行器。
