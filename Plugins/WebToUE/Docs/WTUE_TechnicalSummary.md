...
>
> 当前里程碑：M4——UE 原生表现与合成 8 / 9；M4.8 Tiered Compositing 已完成，M4.9 尚未激活
>
> 当前交付 Profile：PersonalGame-ready 0.5——Win64 项目内生产使用；通用商业 1.0 延后
>
> 最近核验：2026-08-22 `f7f6666`～`aa597fa` 完成四档确定性 Compositing Policy/Plan/Cache、View/Surface lifecycle、Tier0 Direct Paint/Tier1 Material Brush 产品路径及 `CompositingSmoke`；127 / 127 Automation、strict UE 5.8 Win64 Editor Development、Development/Shipping BuildCookRun、双配置独立 Packaged Compositing gate 与冻结三 Corpus Shipping 性能回归全部通过
>
...
| 当前最大风险 | M2 风险已降至当前 Win64 项目内 0.5 可接受等级；M3 基础合同及 M4.1～M4.8 的 Resource/Material/Feedback/Transform/Animation/Transition/Compositing 已收口。M4 剩余风险集中于 M4.9 的大型资源页、首帧/常驻成本与可选 Surface 出口；Behavior/FieldNotify/Command Adapter、Stable Identity 接入和真实双 LocalPlayer/CommonUI Modal 仍开放 | 🟡 |
| 当前策略 | 保持 M2 出口门、M3/ADR-0007 合同边界和已完成的 M4.1～M4.8 纵向闭环；Tier0/1 作为当前产品路径，Tier2/3 在后端不可用时稳定失败关闭。冻结 Corpus 未使用的高级效果与 World Surface 继续 N/A；M4.9 仅作为下一建议，尚未激活 | ✅ |

### 2.2 验证快照

| 项目 | 当前值 |
| --- | --- |
| 自动化测试 | 127 / 127 通过、0 failed、0 skipped（12.657 秒；2026-08-22，M4.8；`StartsWith:WebToUE`）；`CompositingSmokeContract` 聚焦 1 / 1、0 warning（0.018 秒） |
| 当前编译 | M4.8 最终候选 `aa597fa` 的 strict UE 5.8 Win64 Editor Development Operation `f4b6adaccc8c4a49905e1aa5f9b114e5` 完成 70 / 70 actions、warnings-as-errors并启动 PID `11704`；最终 Shipping 发布门恢复 Editor PID `14616`，VibeUE 5.0 readiness/MCP HTTP 200 健康 |
| 当前发布 | tracked Win64 Development/Shipping BuildCookRun Operations `e71b1d1db50e44379c907b3f50ed978b` / `30e4c48987644af394c9cfa9320e66fd` 均通过，各 600 packages、2,269 WebToUE content chunks（含 global 2,270）、251.99 MiB、AutomationTool 最终 ExitCode 0；双配置独立 `CompositingSmoke` gate 与 Shipping 三 Corpus PackagedExitGate 均 `success=true`。受控 fixture 只证明 Tier0/1、K=1、MID/Hit/Clip/Renderer；Tier2/3 明确为 N/A，不外推真实 Portal/离屏成本 |
| 历史发布 | Win64 Game Development/Shipping、BuildCookRun、BuildPlugin 曾通过；发布前须在当前提交重跑 |
| Git 基线 | M2.9 检查点为 `8d26643`、`36fc522`、`e8ac686`、`e524997`、`64c2f7e`、`7df6832`；M3.0 为 `66ada22`、`ecb094c`；M3.1 为 `1fc2a20`、`5342bf6`、`3f1bf20`；M3.2 为 `7be0f8e`；M3.3 为 `692ffeb`、`048a676`、`a7c1db4`；M3.4 为 `9c33768`、`d3e1b80`、`2e0f1c3`；M3.5 为 `4001b08`、`4bdf63c`；M3.6 为 `f05ce52`、`0ae3ac4`；M3.7 为 `21f2933`、`f1c2e01`；M3.8 为 `c3383c6`、`9b666b3`；M3.9 为 `f60b94d`；M4.1 为 `6903e34`、`ccddf69`、`99ad868`、`507c616`、`394cb94`；M4.2 为 `3cce712`、`9449bee`、`be4e728`、`2184640`、`4eacabc`；M4.3a 为 `a046603`、`1df1704`；M4.3b 为 `e4d4ba4`；M4.4 为 `b1a95df`、`ac6314b`、`5730cd3`、`05359ee`；M4.5 实现与门修复为 `f55d4ec`、`b0ad573`、`f6a825c`；M4.6 实现为 `2d2be76`；M4.7 为 `ff9087c`、`1b69e40`、`556bebd`、`b309628`、`96b9eae`、`e3e08a0`；M4.8 为 `f7f6666`、`34265c8`、`1501759`、`5faef14`、`aa597fa` |
...
| M4 UE 原生表现与合成 | 🚧 | 8 / 9 | Texture、Material/MID、Feedback、Transform/Clip、Animation/Transition 与 Tiered Compositing 已完成；剩余 M4.9 性能与 Surface 出口门 |
...
M2 已完成性能可观测性与硬门、完整生命周期分离、类型化样式/选择器/Cascade、共享 Style Template 与稳定身份、Paint-only Pseudo、根 FieldNotify/Text 局部失效、持久 Layout/Resource、可 patch Display List/空间命中、真实 Packaged 渲染、生产宿主和 M2 Go/No-Go。M3.0～M3.7 依次建立 Native Component、Screen Session/Host/Feedback、更新事务、事件/交互身份、Clock/异步取消、属性所有权、多树/Portal 与 Stable Semantic Identity；M3.8 建立 C++ Data/Command Schema 与 `.d.ts` 投影；M3.9 建立 Resource provenance、Document/Route residency、依赖/Manifest seal、Cook freshness 与独立层版本基础合同。M3 当前为 `11 / 11`；这些 Policy 的真实 Compiler/View/Cook/Behavior/Material 产品接入仍按 M4～M6 各自验收，不能把合同完成写成产品能力完成。
...
| 层 | 当前实现 | 已接受的后续边界 |
| --- | --- | --- |
| Compiler | Parser/CSS 前端、RichText lowering、Property ID/Typed Value、Property Metadata、固定槽位 Typed Cascade、Property 应用、Yoga Adapter、C++ Interop Schema snapshot/`.d.ts` 投影；Texture importer 支持 Unreal、相对 Source 与 `generated:`，静态 Material importer 支持 `/Game`/`/Engine` `UMaterialInterface`。受控 CSS `transition` 可把 Opacity/Color/Background/Border/VisualTransform、duration/delay/easing 确定性降低到 Animation IR 1.1，并与 ResourceId/provenance/Brush metadata、BLAKE3-256 closure/freshness 一起 seal | 受限 Behavior/TSX 静态编译消费 Schema；Material 参数作者声明、Behavior IR、Source Map 与完整 Incremental/DDC/Lockfile |
| Asset/IR | 私有扁平 CompiledNodes/Rules、类型化样式声明与 Texture/Material/Font/String Table Resource Manifest、自定义版本 14、原子提交；Texture 与静态 Material ResourceId/provenance/residency/必要 Brush metadata、Resource IR 1.2/freshness stamp、类型化 Visual Transform/Origin，以及版本化 Animation IR 1.1/Transition 表已序列化 | Behavior/Capability IR、Material 参数作者声明与完整多层迁移 |
| Session/Runtime | Screen UI Session 绑定 LocalPlayer、World、Surface、Data/Command、Environment、显式 Game/Unscaled/Real/Test Clock 与 Session Generation；Session-owned 更新协调器在 Game Thread 原子提交，异步协调器提供无默认 Tick 的 Timer/Command Result，Animation 协调器提供 active-only Track、单地址 lease、Retarget/Replace/Cancel、Transition reverse/underlying fill、受控 easing/delay、精确 Clock sample、Generation/Session cleanup 与有界 Trace；Property Ownership Policy 区分 CSS baseline、唯一 Binding/Behavior durable owner、Animation overlay 和 restrictive gate | 把 FieldNotify/类型化 Command/Behavior 接入事务与异步结果，并让 M5/M6 消费 Stable Identity 与多树合同；Keyframes/Layout animation 不进入当前执行器 |
...
| Presentation | Handle-keyed Layout/Text/Paint/Brush、ResourceId-indexed View-owned状态/强对象/异步句柄与可 patch Display List；Critical/Visible/Lazy Resource 边界保持不变；Texture、静态 Material 和 View/Node-owned MID 已接入。Translate/Scale/Rotate/Origin 解析为 affine LocalToView；Opacity/Color/Background/Border/VisualTransform Transition adapter 在同一 Session coordinator 上逐样本应用 overlay、局部 patch Display/空间索引/dirty，并在结束显露最新 underlying，0 Yoga/resource load | Material 参数作者语法与 MID animation adapter、Route、真实 Portal/Overlay、分级 Compositing 与更大资源 Corpus 的真实释放/内存门 |
...
| Tooling | 导入诊断、热重载、Automation、Stat/Trace/Telemetry、纯内存确定性 `.d.ts` emitter、Animation bounded trace、独立 Packaged kernel gate 与版本化 Transition 数值/Paint/Renderer/PNG gate；Cook `PreSave` 重建 Texture/静态 Material expected stamp并以 `WTUE-RES-004` 拒绝 stale last-good | `.d.ts` 磁盘生成、原生 Preview、Runtime Inspector/可视化 Animation Trace、完整 Incremental Compile/DDC/Lockfile、可选 MCP |
...
| `WebToUERuntime` | Runtime | WTUE Document、Runtime Instance、UMG/Slate、绑定、输入、事件、字体、Session-owned 更新/异步/Animation 协调器，以及实验性的 Native Component Registry/Factory/Instance 合同 |
...
Cooked 游戏保留 Compiled Nodes/Rules、Root、Animation IR 1.1/Transition 表、Texture/Font/String Table Resource Manifest、诊断和 Runtime 模块；HTML/CSS 原文、源路径和依赖文件属于 Editor-only 数据。
...
当前已有 Native Component、Session/Host/Feedback、更新事务、事件/身份、Clock/异步、属性所有权、多树/Portal、Stable Identity、C++ Interop Schema，以及 native Animation Kernel 与基础 CSS Transition 产品链。Interop Schema 以项目 C++ descriptor 为唯一事实源，在 Core 生成带 Major/Minor 版本的规范 Data/Command snapshot，并由 Editor-only emitter 单向派生确定性 `.d.ts`；`WTUE-SCHEMA-001..004` 拒绝无效/重复/未知/递归类型、非法 Command shape 和不兼容演进。现有 Binding text/visible/enabled 与字符串 Click/Async token 尚未消费该 snapshot，项目 Schema provider、Context Adapter、Command payload/dispatch、Behavior Compiler、MVVM Adapter、磁盘生成/freshness 都未实现；UI Source/Compiler/现有 View 也尚未创建真实 Portal 或生成/消费 Stable Identity。M4.4 已支持 C++/Host 注入的版本化 Feedback Profile、Critical async gate、确定性 Router 与 UE/项目 backend，但 UI Source/Compiler/Behavior 尚不生成 Cue，也没有扬声器实际出声/声学延迟承诺。M4.7 已支持 Opacity/Color/Background/Border/VisualTransform 的 CSS Transition lowering、受控 easing/delay/retarget/reverse/fill 与 View/Paint adapter；MID target、Keyframes/Layout animation、Portal/离屏合成仍未支持。状态保留式热重载、Feedback 作者语义、触摸/惯性、完整文本编辑/IME、无障碍、组件/列表、Behavior、Material 参数作者语法/`Time`、复杂 CSS、Inspector 和跨平台仍未支持；静态 Material Brush 与 C++ typed Scalar/Vector submission 已按 M4.3 产品边界支持。逐项支持、限制和诊断以 [WTUE_SupportMatrix.md](WTUE_SupportMatrix.md) 为唯一精确来源。
...
| R-05 | Compiler/View 职责集中 | Low | Core 服务、Runtime Presentation、Semantic/Focus/Schema/Animation/Compositing Policy/Plan/Cache、Editor-only `.d.ts` emitter、Game-owned Packaged Runner 与独立出口门脚本已拆分；当前精确发现 127 项 Automation | `SWebToUEView` 继续只编排 Session/Projection/Paint；后续 Surface/Behavior/Component 不得把政策、资源分配和预算重新堆入 Leaf | ✅ Mitigated |
| R-06 | 性能证据和硬门仍不完整 | Medium | 第 6.2 节全部 Enforce 门通过；schema 6 + gate schema 1 覆盖两配置三 Corpus 的 GT/RT/GPU/input、Batch/Vertex、冷启动归因、Development LLM、第二 View 与 K=1 工作量；M4.8 Shipping 回归关键比较最大 `1.496×` | 0.5 维持现有硬门；Shipping LLM、硬件扫描延迟和 Gameface 保持明确不可用/Unknown，不伪装为已测 | ✅ Mitigated |
| R-07 | Map 声明丢失重复属性顺序 | Medium | Core、Compiled IR 与 Hydration 已使用有序声明；当前 127/127 Automation 与 M4.8 Win64 Development/Shipping BuildCookRun 通过 | 保持 Ordered Declaration 专项与资产版本门 | ✅ Mitigated |
...
| R-16 | Transform、Opacity、Material、Clip 与 Filter 需要 Stacking/Compositing，而扁平 Display List 语义不足 | Medium | `f7f6666`～`aa597fa` 建立 Direct Paint→Material Brush→Subtree Layer→Render Target 的 sealed、确定性单向分类；普通/投影敌意原型、完整 teardown、Tier0/1 双配置 packaged fixture、K=1/Renderer/截图均通过。Tier2/3 后端未激活且稳定失败关闭；Portal 作者链、MID animation、Filter/Backdrop 与离屏成本仍无产品证据 | 当前 Profile 只承诺 Tier0/1；Tier2/3、Portal 或独立 Surface 一旦进入冻结 Corpus，必须重新激活实现与 RT/VRAM/输入门，不得把 N/A 或零计数外推为支持 | ✅ Mitigated |
...
| R-18 | 大型组件文档在 View 创建时预载完整 Manifest，形成 I/O、首帧与常驻内存悬崖 | High | M4.1/M4.2 Texture 与 M4.3 Material/MID 已按 ResourceId 消费 Document residency；M4.4 Profile 只异步预取 Critical 组，双配置 Packaged smoke 均证明 6 个 sealed dependency 在交互前从 pending 到 ready、首次派发无同步加载。冻结 Corpus 仍没有大型资源页/音频集，Route、批量释放与内存压力尚无证据 | M4.9 加入大型 Corpus、释放、首帧、PSO/Glyph、音频/图像内存与 Chunk 门 | 🚧 M4 |
| R-19 | TS/资源依赖和 last-good 可能造成不可复现构建或陈旧 IR 进入 Cook | High | M4.1～M4.4 已密封 Unreal/relative/generated Texture、Material 与 Feedback SoundWave/SoundCue/MetaSound/Concurrency 的 Source/saved-package 传递 BLAKE3 closure；Cook freshness 以 `WTUE-RES-004` 拒绝 package drift/stale seal，跨进程专项与双配置真实 Cook 通过 | M5/M6 补 Behavior/Schema、Lockfile/DDC Key、跨机 Incremental/DDC 与 CI | 🚧 M5/M6 |
...
| R-21 | 节点直接播放 UI Sound 会产生重复、错误作用域、首次交互加载尖峰，并把主题/用户设置/项目音频后端耦合进 Behavior | High | M3.1～M3.3 已冻结 Request/Post-Commit 边界；M4.4 又完成版本化 Profile、Critical async gate、Session/LocalPlayer/Viewport/Surface scope、静音/音量、correlation-aware 去重/Cooldown/Throttle、确定性 Variant、Screen/World 意图及默认 UE/项目 backend。Behavior/UI Command 仍不携带 Sound 路径 | M5 实现语义默认、作者覆盖和类型化 `EmitFeedbackCue`；见 ADR-0005 | 🚧 M5 |
| R-22 | C++、UHT/MVVM、`.d.ts` 与 UI Compiler 各自定义协议会产生漂移或 UHT/UBT/Compiler 循环依赖 | High | `c3383c6` + ADR-0009 固定项目 C++ descriptor→Core snapshot→Runtime/Compiler 与 Editor `.d.ts` 的单向链；规范化/演进/投影 3 / 3 专项及 85 / 85 完整 Automation 通过，Runtime 不依赖 Editor/MVVM/生成文件 | M5/M6 实际 Adapter、Compiler 与磁盘 freshness 必须消费同一 snapshot；禁止 `.d.ts` 回读和生成 C++ | ✅ Mitigated |
...
### M3——Runtime Semantics、Host 与原生互操作 ✅ 11 / 11
...
- [x] `P0.5` 选定 C++ Data/Command Schema 单一事实源、版本与 `.d.ts` 生成方向，避免 UHT/UBT/UI Compiler 循环依赖；MVVM 只保留 Adapter 身份。
...
- [x] `P0.5` 冻结 Resource provenance、Document/Route 粒度、Critical/Visible/Lazy 驻留、依赖 Hash、Cook freshness 和多层 IR/Schema 版本合同。
...
- [x] `P0.5` 实现版本化 UI Feedback Profile 和默认注入式 UE Router：Cue 映射到 Sound/SoundCue/MetaSound 或项目 Adapter，覆盖 Critical 预取/Cook、用户设置、Concurrency/Cooldown/Throttle、缺项降级、Screen 2D 与 World Surface 策略；Core 不依赖音频中间件。
- [x] `P0.5` 实现 Translate/Scale/Rotate/Origin 的 Visual Transform、Clip Chain、transformed bounds、逆变换 Hit Test、Semantic Bounds 和空间索引更新。
- [x] `P0.5` 建立原生 Animation IR/Track 与 Active-only Clock；无 Track 时零 Tick，Virtual UI Clock 可在精确时间点验证。
- [x] `P0.5` 将 Opacity/Color/Transform Typed Transition 降低到同一 Animation IR/Track，提供受控 easing、Retarget/Reverse/Cancel/Fill 语义；没有建立独立临时执行器。
- [x] `P0.5` 建立 Direct Paint、复用 M4.3 Material Brush、子树 Compositing Layer、Render Target Effect 四档选择合同；`P0.5-if-used` 只实现冻结 Corpus 使用的 Gradient、Shadow、Nine-slice、Mask/Keyframes，任意 Material `Time` 明确为不受 WTUE 时钟保证的 escape hatch。
...
2026-08-21 的顺序调整不改变 PersonalGame-ready 0.5 Profile、M4 的 9 项宏观验收或跨里程碑前置关系；它只降低单个工作包的跨边界宽度，并让 Animation Kernel 先于 Transition Lowering。2026-08-22 又根据 M4.6/M4.7 的跨进程 freshness、协调器预算、可见失败和门禁成本，把 M4.8 收窄为“Corpus 裁决→有界敌意原型→最低档产品切片→一次候选出口门”。M4.3a/M4.3b、M4.4、M4.5、M4.6、M4.7 与 M4.8 均已完成，宏观 M4 为 `8 / 9`；M4.9 仅为下一建议，尚未激活。

| 顺序 | 微观路线 | 最小纵向退出边界 |
| --- | --- | --- |
| 1 | M4.3a Static Material Brush | `UMaterialInterface` 作者引用→Resource Manifest/Brush metadata→saved-package closure/Cook→共享静态对象→Packaged 可见 Brush；不创建 MID |
| 2 | M4.3b MID Ownership And Parameters | 只在需要不同参数状态时创建 View/Node-owned MID；消费 canonical typed parameter address、事务/属性 owner、GC/释放与跨 View 隔离；不引入动画或 `Time` 保证 |
...
| 7 | M4.8 Tiered Compositing | 先用敌意原型选择最低可行档位，再建立 Direct Paint→Material Brush→Subtree Layer→Render Target 的确定性单向升级合同；四档是选择分类，不等于四套实现。按冻结 Corpus 裁决 Gradient/Shadow/Nine-slice/Mask/Keyframes |
| 8 | M4.9 Performance And Surface Exit | 保持 M2 Screen 门，并让 M4.8 最低产品档位通过真实 ScreenHost 组合回归；只有 Corpus 使用 World Surface 才激活 WidgetComponent RT/Redraw/Gamma/VRAM/输入/可见性门 |

##### M4.7 执行检查点（已完成）

1. **Author/Compiler 合同**：先用红灯锁定受支持 Transition 语法、Opacity/Color/Transform typed target、非法/不兼容值诊断、Animation IR 1.1 lowering、确定性排序与 seal。serialization shape 已 append-only 升级资产版本；没有因 producer fingerprint 变化伪造额外 schema bump。
...
完成证据：持久 `TransitionSmoke` 为 7 nodes/6 rules、5 个 typed Transition、0 resources；Development/Shipping 各在一个目标的合法 K=5 active set 上观察到最大 5 Tracks、251 次 bounded sample、5 次 completion、128 次 committed transaction/property evaluation/state mutation，结束后 active Track/ticker 均释放。4 次 measurement trajectory 各合计 12 Style visits/12 Selector evaluations，即每轨迹 `3/3`；Development/Shipping 分别记录 `2664/2943` 次 Display/空间/dirty patch，Yoga write/dirty/result 与资源加载均为 0。独立 RT/GPU P95 为 `11.967/9.908 ms` 与 `4.949/9.896 ms`，只作该 1920×1080 fixture 的原始 Renderer 观测；两张 PNG 已人工确认完成态，不构成 Portal/离屏合成、硬件 input-to-pixel 或 WTUE↔UMG 等价结论。

`P0.5-if-used` 裁决：基础 Transition 是本路线无条件 `P0.5`，已实现并验收；冻结目标游戏 Corpus 仍不使用 Keyframes、World Surface、Gradient、Shadow、Nine-slice 或 Mask，这些保持有证据的 `N/A`，没有创建占位接口。
...
M4.8 的无条件 `P0.5` 是确定性的 Compositing 分类、所有权、生命周期、失败和预算合同，以及至少一条真实 Slate/Packaged 最低档产品切片；不是预先实现浏览器级合成器。冻结 MainMenu/HUD/ScrollableSettings 继续不使用 Gradient、Shadow、Nine-slice、Mask 或 Keyframes，这些 `P0.5-if-used` 当前均为有证据的 `N/A`；只有 Corpus 事实变化才重新激活，不能用占位 IR、空接口或无消费者 Render Target 冒充进度。

1. **Corpus 快照与有界敌意原型**：先固定三份目标 Source 的能力扫描，并建立两个不进入产品承诺的 Runtime/Automation 原型：普通子树中的重叠 sibling + nested transform/clip/opacity/material，以及同 Session/Surface Overlay Anchor 下的 projected subtree。原型只比较当前 Slate/Display 路径能否表达正确 order、clip、opacity、hit 与 dirty，不修改持久资产 schema、公共作者语法或 Support Matrix。若最低可行方案要求改写 ADR-0007 的 Logical/Paint/Semantic 分界、跨 Surface Portal，或引入新第三方/RHI 后端，停止并请求架构裁决。
2. **确定性四档选择合同**：Tier 0 Direct Paint 只使用现有 Draw Element/Display patch；Tier 1 复用 M4.3 static Material/MID Brush 且不创建离屏面；Tier 2 只在子树必须隔离 opacity/clip/order 时建立 Compositing Layer；Tier 3 仅在效果必须采样已合成子树、backdrop 或独立 surface 时允许 Render Target。选择由 sealed 能力与 Paint projection 决定，不依赖每帧启发式、TMap 顺序或设备偶然状态；请求高档但后端/预算不可用时以稳定诊断失败关闭，不静默退化为语义错误的低档。
3. **职责、所有权与终止矩阵**：分类 Policy、Compositing Plan、Layer/RT Cache 和 Slate 提交职责必须可独立测试；`SWebToUEView` 只保留 Session/Projection/Paint 编排，不同时承担政策、资源分配、缓存和预算。Layer/RT 由 View/Surface 生命周期拥有，节点只以 generation-safe Handle 引用，不新增 per-node UObject/UWidget/Slate Widget 或第二 z-order 事实源。Tier change、Projection revision、节点删除、Generation advance、Host/Surface detach、Session/World teardown 必须释放 layer、surface、cache entry、异步句柄和晚到提交；若敌意原型推翻这条所有权偏好，再以真实替代方案更新 ADR，而不是在实现中暗改。
4. **最低档产品纵切**：原型裁决后只实现满足正确语义的最低档位，并让现有 Display List、Clip/Transform、Material/Animation overlay 和 ADR-0007 Paint projection 通过同一接口消费它。受控 `CompositingSmoke` 必须覆盖重叠顺序、嵌套 clip/transform、局部 opacity、静态 Material/MID 隔离、精确 Hit 与局部 Dirty；可用 C++ fixture 注入同 Session/Surface Overlay projection，但 UI Source Portal 作者语法、M5 动态结构、MID animation、Filter/Backdrop 和 World Surface 不随本路线扩张。
5. **工作量、资源与真实性门**：从第一条产品切片记录 tier decision、active layer/surface、allocate/reuse/release/evict、allocated pixels/bytes、redraw/pass/command、Display/空间 patch、dirty rect、Yoga write/dirty/result、Resource request/sync load、Slate Batch/Vertex 与 GT/RT/GPU。K=1 变化不得默认重绘完整 View；合法 in-flight layer/surface 集合必须由预算完整覆盖。未激活的 Tier 3 报告 `N/A`，不能用零值伪装为已测 RT/VRAM。
6. **验证阶梯与证据清单**：原型阶段只运行修改前红灯、聚焦 Automation、静态/diff 检查与一次必要增量 Editor build，不运行 BCR。产品候选冻结后一次性执行 `git diff --check`、完整 WebToUE Automation、strict UE 5.8 Win64 Editor Development、受影响的 Development/Shipping BuildCookRun、独立 Packaged 数值/视觉/Renderer gate，以及 MainMenu/HUD/ScrollableSettings 既有 ScreenHost/Golden/性能回归。Gate 输出必须生成一个包含 candidate HEAD、Corpus/tier 裁决、Operation ID、工作量、资源、截图/hash 和证据边界的结构化 manifest，Evidence Ledger 只引用该事实，不重复手工转录所有原始日志。

退出时必须分别证明：四档选择与拒绝语义、最低档真实产品路径、Projection/Hit/Clip 正确性、Layer/Surface 生命周期、K=1 局部传播、Packaged 可见结果和 Renderer/资源原始数据。BuildCookRun、Automation、单张截图和 Editor 微基准仍不能互相替代；M4.8 完成后停止，不自动进入 M4.9。

完成证据：冻结三 Corpus 的 Source SHA-256 与能力扫描确认 Gradient/Shadow/Nine-slice/Mask/Keyframes 仍未使用；普通敌意子树和同 Session/Surface projection 原型分别验证 Tier2 分类与投影不自动升档。Policy/Plan/Cache 独立于 View 编排，四档按 sealed requirements 确定性选择；Tier2/3 后端不可用时以 `COMP001..004` 稳定拒绝且不保留旧 Paint/Hit。View/Surface generation、tier/projection change、节点删除、detach/shutdown 的 cache teardown 矩阵通过。

受控 `CompositingSmoke` 为 12 nodes/11 rules/1 Material resource；Development/Shipping 独立 packaged gate 均在 K=1 measurement 记录 1 hit candidate/visit、1 inverse hit、2 exact clip、3 Display patch、3 spatial patch、3 dirty rect、1 MID reuse，Yoga write/dirty/result 与同步 Resource load 均为 0。两配置各记录 22 次 Tier0、2 次 Tier1、0 rejection，Tier2/Tier3 明确输出 `N/A`；124 redraw/pass、1,364 commands，0 active offscreen layer/surface/pixel/byte。两张 PNG SHA-256 均为 `512b94898e1167ed3de084ca7757cc07d6e872b37ba456b037bd4647b9d0b22d` 并已人工核验。Shipping 冻结三 Corpus gate 的 steady/cold 全部比较通过，最大比率为 HUD cold `1.496×`。原始日志与 manifest 见 Evidence Ledger；M4.9 未激活。
...
本路线完成后停止。M3.8 已在后续微观路线完成；M3.7 的历史边界仍不包含 C++ Schema、Resource freshness、Behavior 语法或 M4/M5 产品实现。

### M3.8——C++ Data/Command Schema 与 `.d.ts` 投影 ✅ 7 / 7

`P0.5-if-used` 裁决：`N/A`。本路线是无条件 `P0.5` Compiler/Runtime 前置，不包含由冻结 Corpus 选择性启用的产品能力；真实 Data/Command Context Adapter、Behavior/Binding/Command payload 接入、MVVM Adapter 与文件 freshness 仍由 M5/M6 及下一路线分别验收。

- [x] [ADR-0009](ADRs/ADR-0009-Cpp-Interop-Schema-And-TypeScript-Projection.md) 冻结项目 C++ `FWebToUEInteropSchemaDescriptor` 为唯一事实源；UHT reflection、MVVM、`.d.ts`、UI/Behavior Source 与 IR 都只能消费，不能反向生成/扩张协议。
- [x] Core 值代数覆盖 void/bool/int32/float/string/name/text、Enum/Record、Array/Optional，根 Data Field 声明 observability，Command 声明 request、none/immediate/async、result 与 cancellable；任意 UObject/UFUNCTION、动态 Variant/Map 和递归 Record 拒绝。
- [x] `FWebToUEInteropSchemaPolicy` 生成确定性排序 snapshot；无效 Schema/version/identifier、UE 大小写重复、enum wire 冲突、未知/递归类型和非法 Command shape 以 `WTUE-SCHEMA-001..003` 失败关闭且不泄漏部分快照。
- [x] Major/Minor 演进 Policy 允许同版本完全不变、同 Major Minor additive，拒绝版本倒退、同版本漂移与 Minor breaking change（`WTUE-SCHEMA-004`）；显式 Major 才能承载 breaking contract。
- [x] Editor-only `FWebToUESchemaTypeScriptEmitter` 从 snapshot 纯内存生成 byte-deterministic 只读 Data/Enum/Record/Command metadata `.d.ts`；不写文件、不生成 C++/.generated.h/Build.cs，UI Compiler 后续直接消费 snapshot 而非回读声明文本，MVVM 保持可选 Adapter。
- [x] `InteropSchemaCanonicalization`/`InteropSchemaEvolution`/`Editor.InteropSchemaTypeScript` 聚焦 3 / 3（0.037 秒）；完整 `StartsWith:WebToUE` 85 / 85、0 failed/skipped/warnings（8.155 秒）与 `git diff --check` 通过。
- [x] 受保护 UE 5.8 Win64 Editor Development Operation `089e7534036b4c93a0cc68c4ab3b6a4a` 完成 13 / 13 actions，正常关闭 PID `30200` 并启动 PID `35652`；readiness signal、MCP HTTP 200、Python UE 5.8.1/Project WebToUE 与 `Lvl_TopDown` World 健康。路线不改现有 Session/View/FieldNotify/Command dispatch、资产/Cook/序列化/模块依赖、Runtime 热路径或可见输出，因此视觉、PIE、产品性能、Packaged Runtime 与 BuildCookRun 不适用；真实 Adapter/Behavior/freshness 仍未验证。

本路线完成后停止。M3.9 已在后续微观路线完成；M3.8 的历史边界仍不包含真实 Data/Command Adapter、Behavior/Compiler 接入或 Resource freshness 产品实现。

### M3.9——Resource Provenance、Residency 与 Cook Freshness 合同 ✅ 7 / 7

`P0.5-if-used` 裁决：`N/A`。本路线是无条件 `P0.5` Compiler/Runtime/Cook 前置，不包含由冻结 Corpus 选择性启用的产品能力；真实 Texture/Material/Feedback resource、Route 分组加载、World Surface、Compiler/资产 stamp 与 stale-source Cook 集成仍由 M4/M6 单独验收。

- [x] [ADR-0010](ADRs/ADR-0010-Resource-Provenance-Residency-And-Cook-Freshness.md) 冻结一个逻辑 WTUE Document 的 Resource Contract；ResourceId 不等于 Manifest index/UObject/source path，provenance 只允许 `/Game`/`/Engine` Unreal Asset、相对 Source 与 `generated:`，绝对机器路径、HTTP 和未密封 dependency 拒绝。
- [x] UI/Style/Behavior/Schema/Resource/Generated 输入使用逻辑 ID、Kind 与 BLAKE3-256 content hash 形成确定性 dependency closure；Compiler/options/lock 使用独立 fingerprint，provenance/residency/version 形成 Resource Manifest seal，Runtime/Shipping 不回读 Source。
- [x] Document fallback 与 Route-specific assignment 分离；共享 Resource 可被 Route 提升但不能降级。Critical 在 Document/Route 可交互前满足，Visible 在预测/实际可见时请求并允许确定性 fallback，Lazy 只由显式动作请求；Policy 不提前实现请求、释放、预算或 Chunk。
- [x] UI IR、Resource IR 为必需层，Behavior IR、Animation IR、Interop Schema 可显式 `0.0` 缺席；各层独立 Major/Minor，Runtime 只接受相同 Major 且 producer Minor 不高于 consumer，资产自定义版本与层版本保持分离。
- [x] `FWebToUEResourceContractPolicy` 生成 deterministic snapshot/freshness stamp；Cook expected 与 compiled stamp 只有完全相同才 fresh，Source/Schema/Resource/Compiler/Manifest/version 任一变化以 `WTUE-RES-004` 失败，`WTUE-RES-001..005` 覆盖 domain/provenance、重复 identity、residency、stale Cook 和版本不兼容且失败不泄漏部分快照。
...
### M4.1——Unreal Texture/Brush Resource Contract 产品闭环 ✅ 8 / 8
...
- [x] 资产自定义版本升级，`FWebToUECompiledResource` 与 `UWebToUEDocument` 序列化 Unreal Texture 的 ResourceId、provenance、Document residency、独立层版本和 Cook freshness stamp；不引入 Material/MID、Route、relative/generated 或 P1.0 迁移承诺。
- [x] Editor Importer 对 HTML/CSS 原始输入和 Unreal Asset package-saved content fingerprint 生成确定性 BLAKE3-256 dependency closure，以稳定 logical ID 构建 M3.9 snapshot；非法/缺失/HTTP/机器路径失败关闭且 last-good 不覆盖已密封成功修订。
- [x] `img` 默认按实际可见性请求，显式 `data-ue-residency=critical|visible|lazy` 只改变 Document activation 时机；Critical 在交互前满足，Visible 仅在节点可见时请求，Lazy 只经显式 ResourceId 消费入口请求并保持确定性无图片 fallback。
- [x] Runtime 在 Hydration/View 边界验证 Resource IR 版本和 ResourceId/Manifest/assignment 一致性；继续使用 View-owned async request/strong handle，状态变化与 Paint 热路径保持零同步加载、无 per-node UObject/UWidget/Slate Widget。
- [x] Cook 前用当前 Source、Compiler 和 Unreal Asset package fingerprint 重建 expected stamp；exact match 才允许发布，last-good stale 以 `WTUE-RES-004` 失败。完整 DDC/Lockfile/跨机器 Incremental Compile 仍归 M6。
- [x] Import/asset/runtime/freshness 的修改前失败、修改后通过专项覆盖确定性 ID、重复图片共享、Critical/Visible/Lazy 请求与 fallback、重导入漂移、旧资产重编译和 stale Cook；记录 K=1 资源请求、命中、失败、取消、同步加载与已知所有权。
- [x] `git diff --check`、聚焦与完整 WebToUE Automation、UE 5.8 Win64 Editor Development、tracked Development/Shipping BuildCookRun、真实 Packaged Texture/Brush smoke 和可见截图全部通过；BuildCookRun 不替代 Packaged Runtime、视觉或产品性能结论。
...
### M4.2——Relative/Generated Texture Source 闭环 ✅ 8 / 8
...
- [x] 明确并测试三种作者引用：`/Game`/`/Engine` Unreal Asset、相对 UI Source 的本地图片，以及 `generated:` Unreal 对象引用；HTTP、机器绝对路径、越过项目边界、目录和不支持格式全部失败关闭，Runtime 不读取作者文件。
- [x] 相对图片按规范 logical source identity 生成确定性的 `/Game/WebToUEGenerated/Textures` 资产身份；内容变化原位更新同一对象路径，等价引用去重，机器路径和 content hash 不进入稳定 ResourceId。
- [x] `FWebToUECompiledResource` 序列化验证后的 intrinsic pixel size；未驻留图片也能参与首次 Measure，加载后的 Brush size 必须与密封尺寸一致，尺寸漂移失败关闭而非静默改变布局。
- [x] RelativeSource/Generated provenance、原始图片字节、生成输出与 package dependency 进入同一个 M3.9 dependency closure；Cook expected stamp 对 source、generated asset 或 Compiler drift 保持 exact-match `WTUE-RES-004` 门。
- [x] Document dependency/watch/reimport 覆盖相对图片；合法内容或尺寸变化更新同一 generated asset、Manifest 与 stamp，缺失/损坏/越界输入保留 last-good 但不能 Cook。
- [x] Runtime 继续只消费版本化 ResourceId→soft object→View-owned handle；K=1 首次 Measure、请求、命中、失败、取消、同步加载和已知所有权可复现，不新增 per-node UObject/UWidget/Slate Widget 或默认 Tick。
- [x] 修改前失败、修改后通过的 Import/asset/runtime/freshness 专项覆盖规范化、稳定身份、intrinsic size、重导入、last-good 和 stale Cook；冻结 Corpus 审计明确保持上述 `P0.5-if-used=N/A` 边界。
- [x] 候选冻结后通过 `git diff --check`、完整 WebToUE Automation、UE 5.8 Win64 Editor Development、tracked Development/Shipping BuildCookRun、真实 Packaged relative/generated texture smoke 和可见截图；分别报告 Editor、Cook 与 Packaged Runtime 的证据边界。

退出证据：`StartsWith:WebToUE` 95 / 95、0 failed/skipped（11.934 秒）；Editor Development Operation `f3f26b142791497ab1c78b3c321887a7` 5 / 5 actions；Development/Shipping BuildCookRun `8eafd177f300498eb6e329a212e0fb8f` / `fd28f6aa217b424492dbd3ca620a767d` 均 AutomationTool ExitCode 0。双配置真实 Packaged smoke 各证明 1 个 `RelativeSource`、稳定 generated object path、1254×1254 sealed imported size、主 View 1 次 async request、第二 View 1 次 cache hit、0 同步加载/失败；两张 1920×1080 PNG SHA-256 均为 `848D2466714191AB1AA030DDB13D1D15768294892B8576FB60ECB8CAB221634A`，已目视确认真实 Texture Brush。首次 Development Cook 正确以 `WTUE-RES-004 dependency-closure` 失败，新增跨进程 freshness 测试后改为直接 BLAKE3 saved package bytes，修复候选才通过双配置发布门。该 K=1 fixture 不外推大型资源页、产品级内存/首帧或 WTUE↔UMG 性能等价。

本路线完成后停止，不进入 Material/MID、Feedback、Transform、Animation 或 Compositing。

### M4.3a——Static Material Brush Resource Contract ✅ 7 / 7

本路线已完成。`P0.5-if-used` 审计：冻结 MainMenu/HUD/ScrollableSettings 未使用 Material、Background Image、Gradient、Shadow、Mask 或 Nine-slice，因此额外目标游戏 Material 种类和复杂 Brush metadata 为有证据的 `N/A`；静态 Material Brush 的 Resource/Cook/Runtime 基础合同作为宏观 M4 第三项的无条件 `P0.5`，已用受控 fixture 和真实 Packaged 路径验收。

- [x] 冻结最小作者引用与诊断：只接受 `/Game`/`/Engine` 下可加载为 `UMaterialInterface` 的 Material 或 Material Instance Asset；错误类、缺失对象、HTTP、机器绝对路径和动态下载失败关闭。relative/generated Material Source 不在本微观路线内，使用明确 unsupported 诊断，Runtime 不回读作者文件。
- [x] 为静态 Material 建立独立 Resource Kind、稳定 ResourceId、Unreal Asset provenance、Document residency、Resource IR 1.2 与只含正 Brush Image Size 的必要 metadata；不开放任意 UObject 属性或反射参数。
- [x] dependency closure 以保存后的 Material/MI package bytes 为直接输入，并通过 AssetRegistry 递归覆盖 MI parent、Material Function 和纹理依赖；规范排序、去重和稳定 fingerprint 对真实依赖变化敏感，不使用时间戳或 package-data hash。
- [x] Import/Reimport/last-good 与 Cook `PreSave` 复用单一构建路径；版本 11 持久 fixture 通过跨进程 freshness，source、直接包或传递资源漂移均以稳定 `WTUE-RES-004` 失败。
- [x] Runtime 只经版本化 ResourceId→soft object→View-owned async handle 消费共享静态 `UMaterialInterface`；多个节点/View 共享引擎对象，不创建 MID、per-node UObject/UWidget/Slate Widget 或默认 Tick，热路径零同步加载。
- [x] Slate Paint 使用密封 metadata 绘制真实 `FSlateMaterialBrush`，失败使用确定性无 Brush fallback；K=1 已报告首次请求、cache hit、失败、取消、同步加载、Brush/Display 命令和已知所有权，并保留 PSO/Shader Compile/大型页边界。
- [x] 红绿专项覆盖 author validation、Resource/metadata/version、稳定身份/去重、传递 closure、reimport/last-good、跨进程 stale Cook、直接 package drift、Runtime/多 View/失败边界；`git diff --check`、101 / 101 Automation、Editor Development、双配置 tracked BuildCookRun、独立 Packaged Material smoke 与可见截图全部通过。
...
### M4.4——Feedback Profile And UE Router ✅ 7 / 7

本路线已完成。冻结 MainMenu/HUD/ScrollableSettings 没有 Feedback Cue/Sound 作者声明，额外目标游戏 Cue 集合、World Surface 空间反馈和对应资源/性能工作量为有证据的 `P0.5-if-used=N/A`；版本化 Profile、Critical 资源/Cook、默认 UE Router 与确定性策略本身是无条件 `P0.5`，已由受控 SoundWave/SoundCue/MetaSound fixture 和真实 Packaged 路径验收。Behavior `EmitFeedbackCue`、语义控件默认 Cue 与作者覆盖仍归 M5，未由本路线提前引入。
...
- [x] 持久 Profile/音频 fixture、跨进程 freshness、完整 Automation、UE 5.8 Win64 Editor Development、Development/Shipping BuildCookRun 与独立 Packaged Feedback smoke 全部通过；Packaged 只证明受控资源驻留、路由/策略和进程内 UE backend 调用，不把扬声器实际出声、输入到像素或大型音频 Corpus 成本伪装为已测。

退出证据：实现提交 `b1a95df`、`ac6314b`、`5730cd3`、`05359ee`；持久 Profile `/Game/WebToUEExamples/Audio/DA_WTUE_FeedbackProfile` 在跨 Editor 重启后通过 Critical dependency/freshness 复读。聚焦 Profile/Router/Session/Host 专项 9 / 9，完整 `StartsWith:WebToUE` 111 / 111、0 failed/skipped/warnings（12.989 秒），`git diff --check` 通过。UE 5.8 Win64 Editor Development Operation `9de81432bc9c461cb2feb7ce9f67d547` 13 / 13 actions、warnings-as-errors并启动 PID `16944`。Development/Shipping BuildCookRun Operations `06a30c8e7e3e46fa9bde8e79fa7c4cc7` / `780f658dac874d31b67b8aafa85d9422` 均为 597 packages、2,266 WebToUE content chunks（含 global 2,267）、251.98 MiB、AutomationTool ExitCode 0；它们只证明 Build/Cook/Stage/Pak/IoStore 与 Editor 恢复。独立 Packaged gates `Saved/Evidence/M4.4/Development-20260821T092400Z` / `Shipping-20260821T093100Z` 均 `success=true`：6 个 sealed dependency 从 Critical pending 异步进入 ready，Host 附着后 11 次 requested/committed 中 6 次 routed/backend success，Cooldown/Dedupe/Throttle/Mute/Missing 均产生预期丢弃，播放意图为 Screen 2D。首次 Development gate 仅因脚本硬编码错误 Profile ID 而在成功 Runtime `result.json` 后验证失败；`05359ee` 改为真实 `webtoue.personalgame.default` 并对同一结果 `-ValidateOnly` 通过，失败 gate 保留且不冒充 Runtime 失败。

K=1/受控工作量结论只覆盖单 Profile、5 个 Cue、6 个 sealed dependency、11 次请求/提交与 6 次 backend 调用；无同步加载 API 路径，策略状态和 Trace 有界且随 Session 释放。不外推实际扬声器出声、声学或 input-to-pixel latency、大型音频 Corpus 的 I/O/内存/Chunk/并发成本、World Surface Host/3D 输出或产品级 WTUE↔UMG 性能。路线无可见 UI 变化，因此 PIE/截图/视觉门不适用；未设置产品性能阈值，Packaged correctness 工作量不作为性能等价结论。本轮在 M4.4 停止，下一建议 M4.5 Visual Transform And Clip，未激活。

### M4.5——Visual Transform And Clip ✅ 7 / 7

本路线已完成。Visual Transform/Clip 是无条件 `P0.5`，不以 Corpus N/A 跳过；冻结 MainMenu/HUD/ScrollableSettings 仍未使用 World Surface、Gradient、Shadow、Nine-slice、Mask 或 Keyframes，这些 `P0.5-if-used` 能力继续保持有证据的 `N/A`，且不属于 M4.5。Transition、Animation、Portal 与分级 Compositing 未提前进入。

- [x] append-only `transform` / `transform-origin` Property ID、类型化 affine/symbolic payload 与资产版本 `VisualTransformAndClip`（12）闭环；支持 `none`、px/% `translate*`、`scale*`、deg `rotate` 及 keyword/px/% origin，CSS transform list 按右到左组合，非法单位、空参数、skew 与未支持函数失败关闭。
- [x] Runtime 将节点/祖先 transform、origin 与 scroll 组合为 `LocalToView`/inverse；Clip Chain 保存任意变换 quad、local transform/size 与 AABB，Slate Paint 使用真实 Render Transform 和 arbitrary geometry clip，旋转 clip 走 stencil。
- [x] Display command 区分完整变换后 `Bounds` 与 clip-intersection `VisibleBounds`；Semantic Bounds 使用完整 border-box AABB，空间 broad phase 使用变换且裁剪后的 AABB，精确 Hit/Scroll 逐 quad 检查 clip 后逆变换进本地 border box，屏外裁剪后代仍可被手柄语义导航发现。
- [x] hover 变换变更只 patch 受影响子树的 transform、clip、Display/空间索引与 dirty union；Telemetry schema 14 记录 transform resolve、clip zones、inverse hit 与 exact clip，不写 Yoga style、不 dirty Yoga node、不改变 Layout result。
- [x] 持久 `/Game/WebToUEExamples/TransformClipSmoke` 与 `WebUI/Examples/TransformClipSmoke.html` 覆盖嵌套旋转 overflow clip、变换目标和 hover patch；三个冻结 Corpus、三个既有 Resource smoke 与 generated Texture fixture 均重编译为版本 12，跨 Editor 重启复读且无脏包。
- [x] `VisualTransformClip`、`VisualTransformPatchWorkload`、`TransformClipSmokeContract` 及 parser/property/version/telemetry 邻接专项覆盖任意 quad clip、逆命中、AABB 假阳性拒绝、Semantic/空间边界、真实 Slate stencil 与 K=1 局部工作量；完整 Automation 保持全部历史输入、语义、滚动、资源与发布策略回归。
- [x] `git diff --check`、UE 5.8 Win64 Editor Development、完整 Automation、Development/Shipping tracked BuildCookRun、双配置独立 Packaged Runtime/视觉门和 1920×1080 截图审阅全部通过；BuildCookRun 只证明 Build/Cook/Stage/Pak/IoStore 与 Editor 恢复，Packaged smoke 才证明本 fixture 的真实 Runtime/Slate 输出。

退出证据：实现与门修复提交 `f55d4ec`、`b0ad573`、`f6a825c`；最终 `StartsWith:WebToUE` 114 / 114、0 failed/skipped/warnings（13.426 秒），`git diff --check` 通过。UE 5.8 Win64 Editor Development Operation `0f314907d3864cacb142485719a1782b` 4 / 4 actions、warnings-as-errors并启动 PID `34032`。最终 Development/Shipping BuildCookRun Operations `67b346592661407f8344e6a11fd7b341` / `dbae42f0204149c586551a31b38f7560` 均为 598 packages、2,267 WebToUE content chunks（含 global 2,268）、251.98 MiB、AutomationTool ExitCode 0；最终 Shipping 后 Editor PID `3220` readiness/MCP HTTP 200 健康。

独立 Packaged gates `Saved/Evidence/M4.5/Development-20260821T1202Z` / `Shipping-20260821T1211Z` 均 `success=true`。两者 measurement 都记录 2 次 transform resolve、16 个 clip zones、1 次 inverse hit、2 次 exact clip、8 次 display patch、8 次 spatial patch、8 个 dirty rect，Yoga writes/nodes dirtied/results changed 均为 0；第二 View 为 3 次 transform resolve、9 个 clip zones。两张 PNG 均为 1920×1080、140,250 bytes、SHA-256 `ED94B693EB5B8B918C4F3BF390A1980BCEF5E6BD4E2533E0D30D93DCE49D4D07`，已目视确认嵌套旋转 clip 无 AABB 泄漏或 stencil 伪影。Development product policy 在等价成熟 Paint 状态下比较第一/第二 View known-owned `33,688 / 33,208 B`（比率 0.986，门限 1.10）并通过。

失败证据未隐藏：首个真实 Slate stencil Automation 因测试使用 null PaintWindow 触发 Editor crash，改用真实离屏 `SWindow` 后闭环；首次完整套件 113 / 114 暴露 Semantic Bounds 错用 clipped `VisibleBounds` 导致屏外 Gamepad navigation 失败，`b0ad573` 改为完整 transformed `Bounds` 后 114 / 114；首次 Development packaged gate 的 Runtime 几何/计数通过但 known-owned 产品策略失败，根因是第一 View prepass census 与第二 View post-Paint census 生命周期不等价，`f6a825c` 在不放宽 1.10 门限的前提下对齐成熟状态后通过。

K=1 结论只覆盖 9 个 compiled nodes、7 条 rules、单目标 hover 进入/离开、两个 View 和该固定 1920×1080 D3D12 fixture；Development/Shipping GT/RT/GPU P50 分别为 `4.329/10.252/8.907 ms` 与 `1.415/2.836/7.494 ms`，仅作诊断输入，不外推真实游戏工作量、Portal/World Surface、离屏 Filter、输入到像素或 WTUE↔UMG 产品性能等价。本轮在 M4.5 停止，下一建议 M4.6 Animation Kernel，未激活。

### M4.6——Animation Kernel ✅ 7 / 7

本路线已完成。原生 Animation Kernel 是无条件 `P0.5`，不以 Corpus N/A 跳过；冻结 MainMenu/HUD/ScrollableSettings 不含 Transition/Keyframes，也不使用 World Surface、Gradient、Shadow、Nine-slice 或 Mask，这些选择性能力继续保持有证据的 `P0.5-if-used=N/A`。M4.7 的作者 Transition lowering、Easing/Reverse/Fill 与真实 View/Paint target adapter，M4.8 的 Portal/分级 Compositing 均未提前进入。

- [x] append-only 资产版本 `AnimationIR`（13）序列化 Animation IR 1.0；typed scalar/color/affine transform 值与 Opacity/Color/Background/Border/VisualTransform/Material Scalar/Vector canonical target 失败关闭，稳定诊断为 `WTUE-ANI-001..006`，seal/version mismatch 在 Hydration 前拒绝。
- [x] 每个 UI Session 独占一个 Animation coordinator；只在存在 active Track 时注册 Core ticker，无 Track 时注册/调用均为 0。每次 Pump 的 sample 预算至少覆盖 active Track 数，防止合法工作被内部预算饥饿。
- [x] Track 只读取 Session 注入的 Game/Unscaled/Real/Test Clock；Virtual Test Clock 在精确时间点得到确定性线性 sample，终点精确提交 `To` 值并同步释放 overlay lease，不使用 wall-clock sleep 或逐帧脚本。
- [x] 每个 canonical address 只持有一个 lease；Start、Retarget、Replace、Cancel 共享同一协调器。Retarget 从当前 sample 继续，Replace/Cancel 释放旧 overlay 并显露最新 underlying value；target adapter 替换也会先释放旧适配器持有的 overlay。
- [x] Generation 推进、Session 失活/Shutdown 与 stale/superseded token 同步清理 Track；sample 通过 M3 Session-owned update transaction 原子提交，有界 Trace 记录 start/sample/retarget/replace/cancel/complete/stale/release。
- [x] 七个持久 WTUE Document 与 generated Texture dependency 均通过 Editor API 重编译/保存为版本 13；最终 Animation/CompiledDocument 聚焦 5 / 5、完整 `StartsWith:WebToUE` 117 / 117、`git diff --check` 与 UE 5.8 Win64 Editor Development 均通过。
- [x] Development/Shipping tracked BuildCookRun 与独立 staged-inner-binary Packaged Animation smoke 全部通过；BuildCookRun 只证明 Build/Cook/Stage/Pak/IoStore 与 Editor 恢复，Packaged smoke 只证明受控 native kernel/Clock/lease/transaction/lifecycle，不冒充视觉、Renderer/GPU 或产品性能证据。

退出证据：实现提交 `2d2be76`；最终聚焦 5 / 5、0 failed（0.044 秒），完整 117 / 117、0 failed/skipped/warnings（12.369 秒），`git diff --check` 与 smoke 脚本 PowerShell AST parse 通过。UE 5.8 Win64 Editor Development Operation `d7908002f48a4538817f44eb7fb6e66a` 完成 5 / 5 actions、warnings-as-errors。Development/Shipping BuildCookRun Operations `9ad907d4395f4411828cb837a946810b` / `f21a6db2321d4f4dbbfcbbfc605599de` 均为 598 packages、2,267 WebToUE content chunks（global 2,268）、251.98 MiB、AutomationTool ExitCode 0；最终 Shipping 后 Editor PID `8792` readiness/MCP HTTP 200 健康。

独立 Packaged gates `Saved/Evidence/M4.6/Development-20260822T0159Z` / `Saved/Evidence/M4.6/Shipping-20260822T0205Z` 均 schema 1、`success=true`：Animation IR 1.0、Test virtual clock、idle ticker invocations 0、quarter sample 2.5、retarget midpoint 11.25、Replace/Cancel 后 underlying 77、terminal exact 1、9 次 committed transaction、10 条 bounded trace、3 次 release，并覆盖 Generation stale cleanup 与 Session shutdown。

失败证据未隐藏：首次编译因误用 `FDelegateHandle` 而非 `FTSTicker::FDelegateHandle` 失败；首次 Runtime 专项因无效 `ULocalPlayer` Outer 触发 ensure，改为 Engine-owned fixture；首次完整套件 116 / 117 暴露 Material 文档在 generated Texture 依赖稳定前封存导致 `StaticMaterialCookFreshness` stale，修复为依赖稳定后只重导入/重封存两个 Material 文档。最终门均在这些根因修复后重新执行。

受控工作量只覆盖单 Session、单地址 Track 序列和 K=1 精确 Clock/lease 生命周期：idle ticker 0、9 次事务、10 条 Trace、3 次 release。没有可见 UI 变化，PIE/截图/视觉门不适用；没有设置产品性能阈值，也未测 Renderer/GPU、input-to-pixel、真实 Transition/作者 lowering、Easing/Reverse/Fill、Portal/World Surface 或 WTUE↔UMG 等价。本轮在 M4.6 停止，下一建议 M4.7 Transition Lowering，未激活。
...
| Runtime | `AssetVersion`、`BindingIndex`、`CompiledDocumentBoundary`、`OrderedDeclarationHydration`、`RuntimeIdentity`、`RuntimeInstanceIsolation`、`RuntimeCacheSeparation`、`RuntimePresentationIsolation`、`SemanticIdentityPlan`、`SemanticIdentityFailures`、`ResourceContractCanonicalization`、`ResourceContractFailures`、`ResourceContractHydration`、`AnimationIRContract`、`AnimationTrackClock`、`AnimationLeaseLifecycle`、`TransitionOpacityAdapter`、`TransitionPaintTransformAdapter`、`CompositingTierPolicyPrototype`、`CompositingAdversarialSubtreePrototype`、`CompositingProjectedSubtreePrototype`、`CompositingPlanLifecycle`、`PseudoInvalidationPath`、`PaintOnlyPseudoResourceSafety`、`TypedCascadeSlateOutput`、`TextCacheKeyAndDirtyPath`、`PersistentLayoutState`、`PersistentLayoutDependencies`、`ResourceLifecycle`、`ResourceResidency`、`ResourceIntrinsicSize`、`StaticMaterialLifecycle`、`DynamicMaterialParameterLifecycle`、`FeedbackProfileContract`、`FeedbackProfileFailures`、`FeedbackRouterPolicy`、`FeedbackRouterScopeLifecycle`、`FeedbackRouterLocalPlayerIsolation`、`TextWrapping`、`LocalizedRichText`、`ScrollInteraction`、`SemanticFocus`、`EventPathSafety`、`EventRouting`、`InteractionIdentity`、`TreeProjection`、`PortalFocusRestore`、`NativeComponentRegistry`、`SessionFeedback`、`ScreenHostFeedbackProfile`、`UpdateTransaction`、`UpdateQueue`、`ClockDomains`、`AsyncTimer`、`AsyncCommand`、`AsyncLifecycle`、`ScreenHost`、`GamepadNavigation`、`DpiSafeZone`、`PerformanceInstrumentation`、`PaintOrderCache`、`DisplayListOwnership`、`SpatialPaintHitWorkload`、`DisplayListDebugOverlay`、`DisplayListDebugImage`、`SlateBatchCompatibility`、`VisualTransformClip`、`VisualTransformPatchWorkload` |
| Benchmark | `CompositingCorpusContract`、`CompositingSmokeContract`、`CorpusContract`、`CorpusSlateOutput`、`CorpusDpiGolden`、`CorpusOptionalInputContract`、`CorpusSurfaceContract`、`PackagedExitPolicy`、`ResourceTextureSmokeContract`、`ResourceMaterialSmokeContract`、`ResourceMaterialParameterSmokeContract`、`TransformClipSmokeContract`、`TransitionSmokeContract` |
| Editor | `BenchmarkScenarios`、`BenchmarkStatistics`、`RuntimeHoverBenchmark`、`RuntimeFieldNotifyBenchmark`、`RuntimeHydrationBenchmark`、`RuntimeWarmLayoutBenchmark`、`RuntimeStressLayoutBenchmark`、`RuntimeUnchangedPaintBenchmark`、`BindingImport`、`FieldNotifyInvalidation`、`PropertyOwnershipIntegration`、`TransitionImport`、`InteropSchemaTypeScript`、`LocalizationImport`、`OrderedDeclarationImport`、`ResourceManifest`、`RelativeTextureSource`、`RelativeTextureFailures`、`RelativeTextureWatcher`、`RelativeTextureCookFreshness`、`StaticMaterialSource`、`StaticMaterialFailures`、`StaticMaterialCookFreshness`、`StaticMaterialPackageDrift`、`FeedbackProfileCookFreshness`、`FeedbackProfilePackageDrift`、`ReimportRecovery` |
...
- M4.8 四档是确定性升级分类，不是四套强制实现；先用有界原型证明最低档，再做一条产品纵切。Render Target、Gradient、Shadow、Nine-slice、Mask 和 Keyframes 没有 Corpus 证据时保持 `N/A`，禁止为路线完整感创建占位 schema、空 backend 或未使用 surface。
- M4.8 的原型、源码迭代与发布候选使用分层门禁；只有冻结候选运行完整 Automation/strict Editor build/双配置发布与 Packaged 视觉门。证据 manifest 记录候选和原始路径，避免为文档收口重复 Cook 或手工复制整段日志。
