...
- When creating a persistent Feedback Profile and its referenced audio assets in one Editor script, wait until AssetRegistry can resolve the saved dependencies before rebuilding and saving the profile seal; otherwise the first cross-process freshness check can expose package drift even though the same Editor session appeared valid.
