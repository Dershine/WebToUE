...
- A shell-host timeout can leave `RunUAT`, UBT, or `UnrealEditor-Cmd` children alive after the parent command disappears; inspect the tracked release-host/process-tree PIDs plus persisted UAT logs before retrying, and never start a duplicate release gate merely because the host stopped streaming output.
...
- Keep version reimport and directory watching limited to persistent WTUE assets; transient Automation Documents under `Saved` can delete their sources before the 200ms watcher fires and otherwise leak delayed import errors into later tests.
- Run lifecycle status through Windows PowerShell 5.1 when the host is PowerShell 7; its `ConvertFrom-Json` can auto-convert ISO timestamps before the wrapper parses them again, shifting `sessionStartUtc` by the local offset and producing a false `stale-session` result.
...
- A staged `WebToUE.exe` is a bootstrap launcher and can exit before the packaged runner writes evidence; packaged gates must resolve and wait for `WebToUE\Binaries\Win64\WebToUE-Win64-<Configuration>.exe` when that inner binary exists.
- UE adaptive/non-unity builds require any header that exposes a WebToUE type to include that type's defining header directly; audit header self-containment before a protected rebuild, and on UE 5.8 validate `FLinearColor` finiteness component-wise with `FMath::IsFinite` rather than a nonexistent member helper.
- When one Editor session creates or migrates persistent WTUE consumers together with generated/referenced assets, stabilize and save upstream Source/resources first, wait until AssetRegistry resolves those saved dependencies, then rebuild and seal downstream consumers; same-session success is not cross-process freshness evidence.
- For any bounded stateful coordinator, make capacity budgets internally cover every declared legal in-flight set (for example, animation sample budget must be at least active-track count) and test the full termination matrix: cancel/replace, owner or adapter swap, generation advance, and Session/World teardown must leave no late mutation, lease, ticker, or retained work.
